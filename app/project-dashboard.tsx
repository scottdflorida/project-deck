...
function latestActivity(project: ProjectRecord) {
  const activity = projectActivity(project);
  if (!activity.at && project.transient?.activity === "checking") return { at: null, relative: "Checking files…", exact: "Looking for the newest project-file timestamp.", source: "No Git activity · checking files" };
  if (!activity.at) return { at: null, relative: "No activity found", exact: "No local commit, GitHub push, or project-file timestamp was found.", source: "No commit, push, or file update found" };
  const date = new Date(activity.at);
  if (Number.isNaN(date.getTime())) return { at: null, relative: "Unavailable", exact: "Activity timestamp unavailable", source: "Timestamp unavailable" };
  ...
  const source = activity.source === "local_commit"
    ? "Local commit"
    : activity.source === "file_update"
      ? "Latest project-file update · no Git activity"
      : project.github.state === "matched"
        ? "GitHub push · repository not linked locally"
        : "GitHub push";
  return { at: activity.at, relative, exact: date.toLocaleString(), source };
}
...
function PreferenceDialog({ modal, busy, error, onClose, onConfirm }: { modal: PreferenceModal; busy: boolean; error: ProjectActionFailure | null; onClose: () => void; onConfirm: () => void }) {
  ...
  const title = isIgnore ? active ? "Show in working set" : "Hide project" : active ? hasGithubRepository ? "Resume GitHub sync" : "Allow GitHub" : hasGithubRepository ? "Ignore GitHub sync" : "Keep local only";
  const copy = isIgnore
    ? active ? "This shows the project in the normal working set again. Nothing in its folder will change." : "This hides the project from normal working-set views. It remains on disk, continues to be scanned, and can be found under Hidden projects."
...
function SyncColumnAction({ project, busy, onModal }: { project: ProjectRecord; busy: boolean; onModal: (modal: RepoModal) => void }) {
  ...
  else if (sync.key === "not_checked" && project.github.state === "unavailable") action = <span className="action-guidance">Connect GitHub above to check for a repository</span>;
...
function ProjectRow({ project, githubAvailable, busy, actionError, onAction, onCopy, onModal, onDismissActionError }: { project: ProjectRecord; githubAvailable: boolean; busy: boolean; actionError: ProjectActionFailure | null; onAction: (project: ProjectRecord, kind: "init", trigger: HTMLElement) => void; onCopy: (project: ProjectRecord) => void; onModal: (modal: NonNullable<ModalState>) => void; onDismissActionError: () => void }) {
  const git = gitPresentation(project); const github = githubPresentation(project); const sync = syncPresentation(project); const update = latestActivity(project);
  const SyncIcon = sync.key === "checking" ? LoaderCircle : sync.key === "no_repository" ? Minus : sync.key === "in_sync" ? Check : sync.key === "ahead" || sync.key === "not_pushed" ? ArrowUp : sync.key === "behind" ? ArrowDown : sync.key === "diverged" || sync.key === "history_mismatch" ? AlertCircle : Github;
