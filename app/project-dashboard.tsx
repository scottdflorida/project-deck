...
export function ProjectDashboard() {
  ...
  const chooseRoot = async (useNative: boolean) => { const candidate = rootDraft.trim().replace(/[\\/]+$/u, ""); if (!useNative && data && [data.rootPath, data.rootLabel].some((root) => root.replace(/[\\/]+$/u, "") === candidate)) { setRootPickerOpen(false); setToast(`Already using ${data.rootLabel}.`); return; } setRootBusy(true); setRootError(null); try { const response = await fetch(`${API_BASE}/api/root${useNative ? "/choose" : ""}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: useNative ? undefined : JSON.stringify({ path: rootDraft }) }); const result = await response.json() as RootSelectionResponse | { error: string }; if (!response.ok || "error" in result) throw new Error("error" in result ? result.error : "The folder could not be selected."); if ("unchanged" in result) { setRootPickerOpen(false); setToast(`Already using ${data?.rootLabel || rootDraft}.`); return; } dataRef.current = result; setData(result); setRootDraft(result.rootLabel); setRootPickerOpen(false); setToast(`Folder changed to ${result.rootLabel}.`); } catch (caught) { setRootError(caught instanceof Error ? caught.message : "The folder could not be selected."); } finally { setRootBusy(false); } };
  const openRoot = async () => { if (!data || rootOpening) return; setRootOpening(true); setUtilityError(null); try { const response = await fetch(`${API_BASE}/api/root/open`, { method: "POST" }); const result = await response.json() as { ok?: boolean; error?: string }; if (!response.ok || result.error) throw new Error(result.error || "The folder could not be opened."); setToast(`Opened ${data.rootLabel} in your file browser.`); } catch (caught) { setUtilityError(caught instanceof Error ? caught.message : "The folder could not be opened."); } finally { setRootOpening(false); } };
