...
function SystemIcon({ size = 16, className }: { size?: number; className?: string }) {
  return <svg className={className} width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="8.5"/><path d="M8 12h8M12 8v8"/></svg>;
}
const AlertCircle=SystemIcon,ArrowDown=SystemIcon,ArrowUp=SystemIcon,Check=SystemIcon,CircleDot=SystemIcon,Copy=SystemIcon,ExternalLink=SystemIcon,Folder=SystemIcon,GitBranch=SystemIcon,Github=SystemIcon,LoaderCircle=SystemIcon,Lock=SystemIcon,RefreshCw=SystemIcon,Search=SystemIcon,Unlock=SystemIcon,X=SystemIcon;
...
export function ProjectDashboard() {
  ...
  useEffect(()=>{if(!data?.enriching)return;const id=window.setTimeout(()=>void loadRef.current(false),5000);return()=>window.clearTimeout(id);},[data?.enriching,data?.scannedAt]);
  ...
  return <main className="dashboard-shell"><header className="topbar"><a className="brand" href="#top"><span className="brand-mark"><GitBranch size={15}/></span><span>PROJECT DECK</span></a><div className="topbar-tools"><span className={`connection ${data?.github.available?"online":""}`}><i/>{!data&&error?"Scanner offline":data?.github.available?`GitHub · @${data.github.login}`:"GitHub CLI not connected"}</span><button className="button refresh-button" onClick={()=>{setRootDraft(data?.rootLabel||"~/Documents");setRootError(null);setRootPickerOpen(true);}}><Folder size={15}/> Folder</button><button className="button refresh-button" disabled={refreshing} onClick={()=>void loadProjects(Boolean(data),true)}><RefreshCw className={refreshing?"spin":""} size={15}/>{refreshing?"Checking…":"Refresh"}</button></div></header>
  ...
  <section className="overview" aria-label="Project summary"><div><span>PROJECTS</span><strong>{loading&&!data?"—":counts.all}</strong><small>direct folders</small></div><div><span>ON DISK</span><strong>{loading&&!data?"—":formatProjectSize(counts.totalBytes)}</strong><small>measured total</small></div><div><span>GIT / REMOTE</span><strong>{loading&&!data?"—":`${counts.git} / ${counts.github}`}</strong><small>tracked / linked</small></div><div className={counts.attention?"attention":""}><span>ATTENTION</span><strong>{loading&&!data?"—":counts.attention}</strong><small>{counts.attention?"records to review":"all systems clear"}</small></div></section>
  ...
  <div className="sr-only" role="status" aria-live="polite">{loading?"Scanning project folders":refreshing?"Checking project sizes and GitHub status":toast||""}</div>{toast&&<div className="toast" role="status"><Check size={15}/>{toast}</div>}{modal&&<ActionModal modal={modal} busy={busyProject===modal.project.name} onClose={closeModal} onSubmit={p=>void action(modal.project,modal.type==="create"?"create-repo":"push",p)}/>} {rootPickerOpen&&<RootPicker value={rootDraft} busy={rootBusy} error={rootError} onChange={setRootDraft} onClose={()=>!rootBusy&&setRootPickerOpen(false)} onBrowse={()=>void chooseRoot(true)} onSave={()=>void chooseRoot(false)}/>}</main>;
}
